...
// Lives at the top level (outside the auth-gated `(app)` route group) so
// signed-out visitors land on the public marketing page instead of being
// bounced to /login. For signed-in users it renders the same dashboard
    ...
    <div className="min-h-dvh bg-[var(--bg)]">
      ...
      <main className="w-full">
        {userRituals.length === 0 ? (
          <div
            className="max-w-2xl mx-auto px-4 flex flex-col items-center justify-center min-h-[60vh] text-center gap-6"
          ...
          <div
            className="max-w-2xl mx-auto px-4 flex flex-col gap-6 py-4"
            style={{ paddingTop: 'calc(var(--header-height) + 24px)' }}
          >
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="text-2xl">
                      {ACTIVITY_EMOJI[ritual.activityType] ?? '🧭'}
                    </span>
                    {ritual.logoUrl && (
                      <img
                        src={ritual.logoUrl}
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    {ritual.tagline && (
                      <span className="text-xs text-[var(--fg-muted)] truncate italic">
                        {ritual.tagline}
                      </span>
                    )}
                  </div>
