...
### Done
- [x] **ZKH-44 Read-only crew + shareable ritual view** — sponsors can add placeholder participants (name, optional nickname/nationality/email) who don't need an OAuth account. Schema: `ritual_members.is_placeholder` + `placeholder_created_by`; `rituals.read_only_token` (lazy-generated, rotatable). New `/view/[token]` public route (robots:noindex) shows roster + flight board + expense ledger; Lore stays gated. Claim flow at `/view/[token]/claim`: post-OAuth picker reassigns the stub user's eventAttendees/expenses/awards/lore-mentions in one transaction (`lib/placeholder-claim.ts`); manual path runs the server action, email-match path auto-fires from `auth.ts` signIn callback. Notify-only (no sponsor approval gate) — sponsor gets a Skald-voiced email via `/api/internal/sponsor-claim-notice` (Node runtime, keeps email stack out of the middleware bundle). New "Invite" Call variant (`emails/StageInvite.tsx`, Haiku copy in `lib/ai/call-copy.ts`) fires when sponsor enters a placeholder email. Sponsor UI: new Crew section on Settings (read-only link, add-placeholder form, list with resend/remove), + "Add someone" on the Commitment Board, placeholder badge on the Crew page. Admin Test Harness gets the new `invite_placeholder` variant with a per-ritual placeholder picker. Email collision: refuses to create a stub if the email already maps to a yearout account. Linear: ZKH-44
